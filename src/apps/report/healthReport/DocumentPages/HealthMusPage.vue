...
const getAgeGroupIndex = async (age, minAge = 20, maxAge = 80, step = 7) => {
  ...
  const myPoint = document.querySelector("#page15 .my-point");
  ...
  <div class="custom-page page a4">
    <div class="page-inside">
      ...
      <div class="title-container">
        <div class="title">
          <img src="@/assets/images/pdf/res/img-muscle-score.png" />

          <h3 class="stretch-text">근육 밸런스 분석 지수</h3>
          <span class="stretch-text">
            근육 밸런스 분석 지수는 나의 근육량과 근육 기능 등 종합적인 근육 상태의 균형을 100점 척도로 평가한 지표입니다. 근육 밸런스 분석 지수가 낮으면 신체 활동 능력이 저하되고, 분석 지수가
            높으면 근육량과 기능이 균형 있게 유지되고 있음을 의미합니다. 나의 점수를 확인해보세요
          </span>
        </div>
        <div class="vertical-line"></div>
        <div class="score-container">
          <div class="stretch-text">[나의 근육 밸런스 분석 지수]</div>
          <div class="score-chart">
            <div class="score-div">
              <!-- 점수 그래프 -->
              <ScoreComDocument :pdfScale="props.pdfScale" :value="props.healthScoreData.hq_musBal.score"
                :status="props.healthScoreData.hq_musBal.status" :width="8" :hideScore="true" :right="false" />
            </div>
            <div class="score">
              <div class="score-top stretch-text"
                :class="props.healthScoreData.hq_musBal.status == 1 ? 'green' : props.healthScoreData.hq_musBal.status == 3 ? 'red' : 'yellow'">
                <div class="score-number">{{ props.healthScoreData.hq_musBal.score }}</div>
      ...
      <div class="data-container">
        <div class="subject">
          <h3 class="stretch-text">
            [한국 {{ props.userInfo.user_gender == 2 ? '여성' : '남성' }}의 연령별 근육 밸런스 분석 지수
            분포와 나의 위치]
          </h3>
          <div class="legend">
            <div class="avg"></div>
        ...
        <div class="violin-container">
          <img class="background" src="@/assets/images/pdf/res/violin/mus-violin-m.png" alt=""
            v-if="props.userInfo.user_gender === 1" />
          <img class="background" src="@/assets/images/pdf/res/violin/mus-violin-f.png" alt=""
            v-if="props.userInfo.user_gender === 2" />
          <div class="violin" id="VolinBox">

            <div class="my-point" :style="{
              // left: myPointX,
              top: 100 - props.healthScoreData.hq_musBal.score + '%',
            }">
              ...
              <div class="my-point-score stretch-text">
                {{ props.healthScoreData.hq_musBal.percent }}등
              </div>
            </div>
          </div>
          <div class="label-y stretch-text">근육 밸런스 분석 지수</div>
          <div class="label-x stretch-text">연령대</div>
        </div>
      </div>
      <div class="desc">
        {{ props.healthScoreData.hq_musBal.msg }}
      </div>
      <hr />
      <div class="bottom-info">
        *본 서비스는 질병의 예방 또는 진단을 목적으로 제공되는 서비스가 아닙니다. 본인의 건강 상태에 대한 정확한 진단이 필요하신 경우 전문의와 상의하시기 바랍니다. 근육 밸런스 분석 지수는 건강검진 결과와 생활 습관 등 다양한 요소 간의 상관관계를 종합적으로 고려하여 산출됩니다. 따라서 입력한 결과가 정상 범위 내에 있더라도 다른 건강한 사람과 비교하여 나의 건강 위험도가 높을 수 있습니다. 
      </div>
    </div>
  </div>
</template>
