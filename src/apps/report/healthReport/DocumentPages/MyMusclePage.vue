...
<template>
  <div class="custom-page a4">
    <div class="page-inside">
        ...
        <div class="data-box">
          ...
          <div class="right">
            <div class="table-container first">
              ...
              <template v-for="(item, index) in props.healthScoreData.bodypartData" :key="index">
                <div class="content-title">
                  ...
                  <!-- <span :class="item.status == 3 ? 'stretch-text danger' : 'stretch-text'">{{ item.name }}</span> -->
                   <span class="stretch-text">{{ item.name }}</span>
                </div>
                <div class="content">
                ...
                <div class="content">
                  <!-- <span :class="item.status == 3 ? 'stretch-text danger' : 'stretch-text'">{{ item.percent }}</span> -->
                   <span class="stretch-text">{{ item.percent }}</span>
                </div>
                <div class="content">
                  <!-- <span :class="item.status == 3 ? 'stretch-text danger' : 'stretch-text'">{{ item.value }}</span> -->
                  <span class="stretch-text">{{ item.value }}</span>
                </div>
              </template>
              <!-- <div class="content-head first">
              ...
              <div class="content-head">
                <span class="stretch-text"
                  :class="props.healthScoreData.hq_musMass.t_status == 1 ? 'normal' : 'danger'"> {{
                    props.healthScoreData.hq_musMass.t_status == 1 ? '평균 이상' : '평균 미만' }} </span>
              </div>
              <div class="content-head">
              ...
              <div class="content-head">
                <span class="stretch-text"> 100명 중 {{ props.healthScoreData.hq_musMass.percent }}등 </span>
              </div> -->
            </div>
            <div class="table-container second">
              ...
              <div class="content">
                <span
                  :class="props.healthScoreData.hq_musMass.fatData.bf_status == 3 ? 'stretch-text danger' : 
                          props.healthScoreData.hq_musMass.fatData.bf_status == 2 ? 'stretch-text warn' : 'stretch-text normal'">
      ...
      <div class="desc">
        <p class="bold">(1) 근육 밸런스 분석 지수에 영향</p>
      ...
      <div class="bottom-info">
        *본 서비스는 질병의 예방 또는 진단을 목적으로 제공되는 서비스가 아닙니다. 본인의 건강 상태에 대한 정확한 진단이 필요하신 경우 전문가와 상의하시기 바랍니다. 나의 부위별 근육량 분포와 부위별 근육량
        판정 및
        부위별 근육량%(kg)은 체성분분석기의 측정 및 분석 내용을 따릅니다.
      </div>
    </div>
  </div>
</template>
...
.table-container {
  ...
  .content-title {
    ...
    width: 100%;
    height: 100%;
    border-bottom: calc(0.28px * $pdf-scale) solid #000000;
    font-weight: 300;
