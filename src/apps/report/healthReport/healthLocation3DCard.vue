...
import Bubble3DChart from "@/components/chart/Bubble3DChart.vue";
import {
  Chart,
  ArcElement,
  Tooltip,
...
Chart.register(ArcElement, Tooltip, Legend, RadialLinearScale, Filler);
...
<template>
  ...
  <cardComponent :title=props.title :more="true" :description-title="'건강 위치란?'"
    ...
    <template v-slot:content v-if="isCardFliped == false">
      <div class="card-content-title first">몸 속 산화 스트레스, 대사 능력, 근육 건강을 고려한 건강 상태를 확인해 보세요.</div>
      <div class="chart-wrap bubble-3d-chart" v-if="datas != null">
        <Bubble3DChart :oxi="datas.hq_oxi.score" :met="datas.hq_met.score" :mus="datas.hq_musBal.score" />
        <div class="legend-container">
          ...
          <div class="legend4"></div><span class="stretch-text">{{ props.userInfo.user_name }}님</span>
        </div>
            ...
            <div class="description-box">
              ...
              <div class="description">
                <p>노화 억제 분석 지수는 {{ datas.hq_oxi.score }} 점이고, 만성질환 억제 분석 지수는
                  {{ datas.hq_met.score }}점이며, 근육 밸런스 분석 지수는 {{ datas.hq_musBal.score }} 점으로
                  {{ datas.health_status_color }} 영역에 위치합니다.</p>
              </div>
              <div class="description">
