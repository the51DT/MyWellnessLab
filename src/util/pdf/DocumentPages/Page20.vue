...
function isAbnormal(val, listData) {
  ...
  if(val == 'CREA') {//크레아티닌은 기준이 다르다
    rst = '';    
     if(listData.isUp && rstData.value[val].val > 1.5) {//초과
      rst = 'red';
     }else if(!listData.isUp && rstData.value[val].val < 0.5) {//미만
      rst = 'red';
     }
  }
