    ...
    <div class="page-inside">
      ...
      <div class="title-container">
        ...
        <span class="stretch-text">
          ...
          (생활습관)를 바탕으로 판정됩니다. 나의 건강 위험 요인을 확인해 보세요.
        </span>
      </div>
      ...
      <div class="data-container">
        <template v-for="(list, i) in list_data" :key="i">
          ...
          <div
            class="description" :class="isAbnormal(list.var, list)">
            <span class="stretch-text">
              {{ list.description }}
            </span>
            <ul>
...
import { comphSm } from "@/assets/js/chart/rstText";
// 해당 페이지 표현해야할 항목, 텍스트
...
const rstData = computed(() => {
  ...
  if(props.comph.resultDto.comData !== undefined) {
    ...
    for (let key in comphSm) {
      key = key.replace('CK_', '');
      if(key == 'eq5d') key = key.toUpperCase();
      obj[key] = result[key];
    }
  }
...
function isAbnormal(val) {
  let rst = '';
  val = val.replace('CK_', '');
  if(val == 'eq5d') val = val.toUpperCase();

  if(rstData.value[val] !== undefined) {//ariData, ariStatus 등에서는 waterAmt를 측정하지 않아서 제외한다.
