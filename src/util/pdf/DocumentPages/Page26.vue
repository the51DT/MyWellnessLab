      ...
      <div class="data-container">
        <div class="data">
          <div class="item1 interest">
            <span class="stretch-text">
              {{ userInfo.name }}님은 {{ interestList.length }}개 분야에 건강
              관심을 가지고 있습니다.
            </span>
        ...
        <div class="data">
          <div class="item1 danger">
            <span class="stretch-text">
              {{ userInfo.name }}님의 건강신호 네트워크에 따르면
              {{ dangerList.length }}가지 건강분야 에 적색불이 밝혀졌으므로 가장
              신경써야 합니다.
            </span>
            <div class="name stretch-text">위험</div>
          </div>
          <div class="item2 stretch-text">
        ...
        <div class="data">
          <div class="item1 warning">
            <span class="stretch-text">
              {{ userInfo.name }}님의 건강신호 네트워크에 따르면
              {{ warnList.length }}가지 건강분야 에 노란불이 밝혀졌으므로 주의
              하여야 합니다.
            </span>
...
function initClusterStateData() {
  if (props.recommendData !== undefined) {
    ...
    props.recommendData.resultDto.MHN_CL.forEach((element, index) => {
      // return this.recommendData.MHN_CL[clusterIndex].Cluster_State;
      let name = "";
      let includeSex = true;
