<template>
  <div class="chart-wrap">
    <div class="top">
      <div class="check-box">
        <img v-if="data.status !== 1" class="check" src="/img/pdf/res/tracking-check.png" />
      </div>
      <div class="name stretch-text">{{ data.name }}</div>
    </div>
    <div class="graph">
      <div class="my-point-pointer">
        <div class="pointer" :style="{ left: data.meanPer + '%' }"></div>

        <img class="my-point-img" src="/img/pdf/res/age-graph-icon.png" alt="" :style="{ left: data.myPer + '%' }"
        />
      </div>
      <!-- 그래프 그라데이션 이미지 -->
      <img :src="'/img/pdf/res/tracking-' + data.barType + '.png'" />
    </div>
  </div>
</template>
