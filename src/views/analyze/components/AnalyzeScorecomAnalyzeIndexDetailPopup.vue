      ...
      if ((index >= ((dataPaging.value - 1) * chartMaxSize)) && (index < (dataPaging.value * chartMaxSize))) {
        switch (props.compId) {
          case 'AnalyzeAgingInhibitionAnalyzeIndex':
            arr.push(item.OXI != null ? item.OXI.toFixed(0) : 0)
            break
          case 'AnalyzeChronicDiseaseControlAnalyzeIndex':
            arr.push(item.MET != null ? item.MET.toFixed(0) : 0)
            break
            // 사용 x 
          case 'AnalyzeAgingSpeedAnalyzeIndex':
            arr.push(item.AgingRate != null ? item.AgingRate.toFixed(0) : 0)
            break
          case 'AnalyzeMuscleBalanceAnalyzeIndex':
            arr.push(item.MUSBAL != null ? item.MUSBAL.toFixed(0) : 0)
            break
            // 사용 x 
          case 'AnalyzeDietIndex':
            arr.push(item.RFS != null ? item.RFS.toFixed(0) : 0)
            break
            // 사용 x 
          case 'AnalyzeExerciseIndex':
            arr.push(item.EXERCISE != null ? item.EXERCISE.toFixed(0) : 0)
            break
            // 사용 x 
          case 'AnalyzeSleepIndex':
            arr.push(item.SH != null ? item.SH.toFixed(0) : 0)
            break
        }
      }
    })
  ...
  if (defData.value != null) {
    defData.value.forEach((item, index) => {
      if ((index >= ((dataPaging.value - 1) * chartMaxSize)) && (index < (dataPaging.value * chartMaxSize))) {
        switch (props.compId) {
          case 'AnalyzeAgingInhibitionAnalyzeIndex':
            arr.push(item.OXI_MEAN != null ? item.OXI_MEAN.toFixed(0) : 50)
            break
          case 'AnalyzeChronicDiseaseControlAnalyzeIndex':
            arr.push(item.MET_MEAN != null ? item.MET_MEAN.toFixed(0) : 50)
            break
            //사용 X
          case 'AnalyzeAgingSpeedAnalyzeIndex':
            arr.push(0.9)
            break
          case 'AnalyzeMuscleBalanceAnalyzeIndex':
            arr.push(item.MUSBAL_MEAN != null ? item.MUSBAL_MEAN.toFixed(0) : 50)
            break
            //사용 x 
          case 'AnalyzeDietIndex':
            arr.push(item.RFS_MEAN != null ? item.RFS_MEAN.toFixed(0) : 50)
            break
            // 사용 x 
          case 'AnalyzeExerciseIndex':
            arr.push(item.EXERCISE_MEAN != null ? item.EXERCISE_MEAN.toFixed(0) : 50)
            break
            // 사용 x 
          case 'AnalyzeSleepIndex':
            arr.push(item.SH_MEAN != null ? item.SH_MEAN.toFixed(0) : 50)
            break
        }
      }
    })
