<template>
  <div class="chart-wrap">
    <!-- <h3>{{ name }}</h3> -->
    <div class="y-axis stretch-text">{{ $t('HistoryGraph.text1') }}</div>
...
function getMarkArea (refHCR) {
  ...
  if (
    ...
    markArea.data.push([
      {
        coord: [0, refHCR.warning_range_min ? refHCR.warning_range_min : refHCR.normal_range_max],
        itemStyle: { color: colorDanger.value },
  ...
  // 주의 위험 모두 존재
    ...
    markArea.data.push([
      ...
      { coord: [100, refHCR.danger_range_min ? refHCR.danger_range_min : refHCR.warning_range_max] }
    ])
    // 정방향
  ...
  markArea.data.push([
    ...
    { coord: [100, refHCR.warning_range_min ? refHCR.warning_range_min : refHCR.normal_range_max] }
  ])
...
onMounted(() => {
  ...
  props.data.forEach((element) => {
    const dataVal = parseInt(element[props.var])
  ...
  props.data.forEach((element, index) => {
    const dataVal = Number(parseFloat(element[props.var]).toFixed(1))
    ...
    date.push(element.checkDate + index)
  })
  if (needDummy) {
    date.push('')
  }
