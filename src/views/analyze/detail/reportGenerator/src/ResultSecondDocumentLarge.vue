<template>
  <div class="graph">
    <div class="label-y">{{ $t('ResultSecondDocument.text1') }}</div>
    <div class="label-x">{{ $t('ResultSecondDocument.text2') }}</div>
    <div class="label-dst">{{ $t('ResultSecondDocument.text3') }}</div>
    <img
      class="graph-back"
    ...
    <div class="pointer" :style="{ left: getAxis().x + '%', top: '112%' }">
      <span class="stretch-text">{{ age.current }}{{ $t('Common.age2') }}</span>
    </div>
    ...
    <div
      ...
      :style="{
        ...
        width: getAxisCurrent().isRight
          ? getAxisCurrent().x + '%'
          : getAxisCurrent().x + getAxisCurrent().width + '%',
        height: getAxisCurrent().height + '%',
      }" />

    <div
      class="pointer"
      :style="{
        left: 18 * props.pdfScale + 'px',
        // top: '87%',
        bottom: getAxisCurrent().height + '%',
      }">
      <b class="stretch-text"> {{ age.health }}세 </b>
    </div>
  </div>
</template>
 ...
 */
const getPointPos = () => {
  const rate = (age.value.current - 19) / 61
...
function getAxisCurrent () {
  const x_value = 85.2 - 13.33 // 13.33=그래프 좌측으로 붙인 x값
  ...
  const startY = 77
  const yDiff = 17
  const rate = (age.value.health - 19) / 61
  // rate = (80 - 19) / 61;
...
.graph {
  ...
  /* width: 283px;
  height: 187px; */
  width: calc(333px * $pdf-scale);
  height: calc(236px * $pdf-scale);
  /* margin: 0 auto; */
  margin-left: calc(74px * $pdf-scale);
  ...
  .reage-graph {
    ...
    position: absolute;
    top: calc(27px * $pdf-scale);
    left: 0;
    /* transform: translateX(); */
    overflow: hidden;
    width: calc(333px * $pdf-scale); /* height: 100%; */
    height: calc(333px * $pdf-scale);
    img {
      position: absolute;
      top: 0;
      left: calc(-13.33px * $pdf-scale);
      /* left: 0; */
      /* top: 0;
      left: 0; */
      width: calc(333px * $pdf-scale);
...
.graph-back {
  position: absolute;
  top: calc(11px * $pdf-scale) !important;
  left: calc(-0.5px * $pdf-scale) !important;
  width: calc(333px * $pdf-scale) !important;
  /* height: calc(236px * $pdf-scale); */
  height: auto;
  /* height: calc(100px * $pdf-scale) !important; */
  /* object-fit: fill; */
  object-fit: contain;
}
...
.my-point-pointer {
  ...
  .my-point-dot {
    bottom: calc(-6px * $pdf-scale);

    width: calc(4.6px * $pdf-scale);
    height: calc(4.6px * $pdf-scale);
    transform: translate(-50%, -50%);
    border-radius: 50%;
    background-color: white;
    border: calc(0.3px * $pdf-scale) solid #00b4de;
...
.my-point-img {
  ...
  z-index: 100; */
  width: calc(17px * $pdf-scale) !important;
...
.my-point-img {
  ...
  /* transform: translate(-50%, -80%) !important; */
  z-index: 100;
  left: calc(-17px / 2 * $pdf-scale) !important;
