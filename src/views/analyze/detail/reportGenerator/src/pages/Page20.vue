...
// 해당 페이지 표현해야할 항목, 텍스트
import { rst3 } from '@/assets/js/chart/documentText'
 ...
 */
function isAbnormal (val, listData) {
  let rst = ''
  ...
  // 혈청 크레아티닌은 기준이 다르다 (높을때와 낮을때)
  if (val === 'CREA') rst = isCREA(val, listData)
 ...
 * @param {*} val
 * @param {*} listData
 * @returns {string} - red or ''
 */
const isCREA = (val, listData) => {
  let rst = ''

  if (listData.isUp && rstData.value[val].val > 1.06) { // 초과
    rst = 'red'
  } else if (!listData.isUp && rstData.value[val].val < 0.6) { // 미만
    rst = 'red'
  }
    ...
    <div class="page-inside">
      ...
      <div class="title-container">
        <div class="title">
          ...
          <h3 class="stretch-text">{{ $t('Page20.text1') }}</h3>
        </div>
      ...
      <div class="legend-container">
        ...
        <span class="stretch-text">{{ $t('Page20.text3') }}</span>
        <div class="legend2">
          <img src="/img/pdf/res/ico_manage.svg" alt="" />
        </div>
        <span class="stretch-text">{{ $t('Page20.text4') }}</span>
      </div>

      <div class="data-container">
        <template v-for="(list, i) in listData" :key="i">
          <div class="name" :class="isAbnormal(list.var, list)" v-if="list.sex ? list.sex === userInfo.sex : true">
            <span class="stretch-text">
              {{ list.name }}
            </span>
          </div>
          <div class="description" :class="isAbnormal(list.var, list)" v-if="list.sex ? list.sex === userInfo.sex : true">
            <span class="stretch-text">
              {{ list.description }}
            </span>
            <ul>
