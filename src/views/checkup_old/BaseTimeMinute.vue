  ...
  watch: {
    ...
    hour(){
      ...
      if(this.limitEight && Number(this.hour) * 60 + Number(this.minute) >= 480) {
        this.validationTxt = "시간 합산은 8시간(480분)을 초과할 수 없습니다.";
        this.hour = ""
        this.minute = ""
        return;
      }
      this.validationTxt = ""
      this.$emit("inputTxtHour", {index: this.index, value: this.hour}) /*부모에게 전달*/
    },
    minute(){
      ...
      this.validationTxt = ""
      this.$emit("inputTxtMinute", {index: this.index, value: this.minute}) /*부모에게 전달*/
    },
    focus(){
      switch (this.focus) {
        case "hour":
  ...
  <div class="align--between BaseTimeMinute">
    <div>
      <label :for="id + index" class="BaseTimeMinute--label">
        <input v-model="hour" type="number" ref="hour" @input="inputTxt($event, `hr`)" name="" min="0" step="1" :id="id + index" placeholder="0" class="BaseTimeMinute--input">
        <span class="BaseTimeMinute--txt">시간</span>
      </label>
    </div>
    <div>
      <label :for="id + '2' + index" class="BaseTimeMinute--label">
        <input v-model="minute" type="number" ref="minute" @input="inputTxt($event, 'mnt')" name="" min="0" step="1" :id="id + '2' + index" placeholder="0" class="BaseTimeMinute--input">
